...
## React to magnet state changes. Resets counter on IDLE, hides during IDLE/CHECK.
func on_magnet_state_changed(_old_state: int, new_state: int) -> void:
	# Show counter only while diving (SINKING=1, RETRIEVING=2)
	_attach_label.visible = new_state == 1 or new_state == 2
